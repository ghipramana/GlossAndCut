...
const createQueue = async (req, res) => {
  try {
    ...
    if (start_time < new Date()) {
      return res.status(400).json({ success: false, message: 'Waktu pemesanan tidak boleh di masa lalu.' });
    }
    ...
    let opStartStr = '08:00';
    let opEndStr = '22:00';
    settings.forEach(s => {
      if (s.key === 'operational_start') opStartStr = s.value;
    ...
    if (bookingStartMinutes < opStartMinutes || bookingEndMinutes > opEndMinutes) {
      return res.status(400).json({
        success: false,
        message: `Jam operasional salon adalah pukul ${opStartStr} - ${opEndStr}. Silakan pilih waktu lain.`
      });
    }
    ...
    // Constraint 1: Customer multi-booking Overlap Validation
    const customerOverlapQueue = await prisma.queue.findFirst({
      where: {
        id_user: id_user,
        status: { in: ['PENDING', 'IN_PROGRESS'] },
        AND: [
    ...
    if (customerOverlapQueue) {
      return res.status(400).json({
        success: false,
        message: 'Anda sudah memiliki booking lain di jam yang sama. Silakan pilih waktu lain.',
        active_queue: customerOverlapQueue
      });
    }
...
/**
 * Get compiled weekly and stylist reports for the Owner
...
const getReports = async (req, res) => {
  try {
    ...
    // Fetch this week's queues
    const weekQueues = await prisma.queue.findMany({
      where: {
        created_at: {
          gte: weekStart,
          lte: weekEnd
    ...
    // Compute Weekly Revenue (completed queues this week)
    const completedQueues = weekQueues.filter(q => q.status === 'COMPLETED');
    const weeklyRevenue = completedQueues.reduce((acc, q) => acc + q.service.price, 0);

    // Statistik Reservasi
    const totalReservations = weekQueues.length;
    ...
    const finalCompletionRate = Math.round(completionRate);

    // Layanan Terlaris (Most Popular Service) & Tingkat Kepopuleran Layanan
    const serviceCounts = {};
    weekQueues.forEach(q => {
    ...
    const activeStaff = stylists.filter(s => s.status === 'AVAILABLE').length;
    
    // For productivity: ratio of completed vs total queues this week
    const stylistPerformance = stylists.map(stylist => {
      const stylistQueues = weekQueues.filter(q => q.id_stylist === stylist.id_stylist);
    ...
    const prevWeekQueues = await prisma.queue.findMany({
      where: {
        created_at: {
          gte: prevWeekStart,
          lte: prevWeekEnd
    ...
    const prevWeekRevenue = prevWeekQueues.filter(q => q.status === 'COMPLETED').reduce((acc, q) => acc + q.service.price, 0);
    const prevWeekReservations = prevWeekQueues.length;

    let revenueDiffPercent = null;
    if (prevWeekRevenue > 0) {
      const diff = ((weeklyRevenue - prevWeekRevenue) / prevWeekRevenue) * 100;
      revenueDiffPercent = diff >= 0 ? `+${Math.round(diff)}%` : `${Math.round(diff)}%`;
    }

    let customersDiffPercent = null;
    if (prevWeekReservations > 0) {
      const diff = ((totalReservations - prevWeekReservations) / prevWeekReservations) * 100;
      customersDiffPercent = diff >= 0 ? `+${Math.round(diff)}%` : `${Math.round(diff)}%`;
    }
...
const getBarberStats = async (req, res) => {
  try {
    ...
    // 1b. Total all time queues taken
    const totalAllTasks = await prisma.queue.count({
      where: {
        id_stylist: stylist.id_stylist
      }
    });

    const completionRate = totalAllTasks > 0 ? Math.round((totalServed / totalAllTasks) * 100) : 0;
